...
const communityDiscussionSchema = new mongoose.Schema({
  ...
  // New fields for chat termination functionality
  isTerminated: {
    type: Boolean,
    default: false
  },
  ...
  // For tracking last activity to determine old chats
  lastActivityAt: {
    type: Date,
    default: Date.now
  },
...
communityDiscussionSchema.index({ isTerminated: 1, lastActivityAt: -1 });
...
// Method to terminate a chat
communityDiscussionSchema.methods.terminateChat = function(reason) {
...
communityDiscussionSchema.methods.updateLastActivity = function() {
  this.lastActivityAt = Date.now();
  return this.save();
};
