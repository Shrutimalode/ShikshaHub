const WebsiteDiscussion = require('../models/WebsiteDiscussion');
...
exports.createDiscussion = async (req, res) => {
  try {
    console.log('=== CREATE WEBSITE DISCUSSION REQUEST ===');
    ...
    if (parentDiscussionId) {
      ...
      parentDiscussion = await WebsiteDiscussion.findById(parentDiscussionId);
      if (!parentDiscussion) {
    ...
    const newDiscussion = new WebsiteDiscussion({
      author: userId,
      authorRole: userRole,
    ...
    console.error('=== CREATE WEBSITE DISCUSSION ERROR ===');
...
exports.getWebsiteDiscussions = async (req, res) => {
  try {
    ...
    // Get discussions
    const discussions = await WebsiteDiscussion.find(filterQuery)
    ...
    await Promise.all(
      discussions.map(discussion => 
        WebsiteDiscussion.findByIdAndUpdate(
          discussion._id,
          { $inc: { viewCount: 1 } },
    ...
    const totalDiscussions = await WebsiteDiscussion.countDocuments(filterQuery);
    ...
    console.error('Get website discussions error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};
...
exports.getDiscussionById = async (req, res) => {
  try {
    ...
    const discussion = await WebsiteDiscussion.findById(discussionId)
      .populate('author', 'name email role')
      .populate('referencedMaterials.material', 'title fileType originalFileName fileUrl')
...
exports.markAsAnswer = async (req, res) => {
  try {
    ...
    const parentDiscussion = await WebsiteDiscussion.findById(discussion.parentDiscussion);
...
exports.searchDiscussions = async (req, res) => {
  try {
    ...
    const discussions = await WebsiteDiscussion.find(query)
      .populate('author', 'name email role')
...
exports.getDiscussionStats = async (req, res) => {
  try {
    ...
    const totalDiscussions = await WebsiteDiscussion.countDocuments({
      parentDiscussion: null,
      isDeleted: false
    });

    const totalQuestions = await WebsiteDiscussion.countDocuments({
      parentDiscussion: null,
      isQuestion: true,
      isDeleted: false
    });

    const answeredQuestions = await WebsiteDiscussion.countDocuments({
      parentDiscussion: null,
      isQuestion: true,
      isDeleted: false,
      replies: { $ne: [] }
    ...
    const totalReplies = await WebsiteDiscussion.countDocuments({
