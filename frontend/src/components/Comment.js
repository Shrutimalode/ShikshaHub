...
const Comment = ({ comment, blogId, onCommentUpdate, onCommentDelete, onReplyAdded }) => {
  ...
  const ReplyTree = ({ reply, depth = 0 }) => {
    ...
    const isReplyLiked = reply.likes && reply.likes.includes(user.id);
    
    // Limit nesting depth for visual clarity
    const maxDepth = 5;
    const currentDepth = Math.min(depth, maxDepth);
    const marginLeft = currentDepth * 20; // 20px per level of nesting
    
    return (
      <div style={{ marginLeft: `${marginLeft}px` }}>
        <Card className={`mb-2 shadow-sm ${reply.isFacultyReply ? 'faculty-comment' : 'border-0'} ${reply.isPinned ? 'pinned-comment' : ''} ${reply.isVerifiedAnswer ? 'verified-answer' : ''}`}>
          <Card.Body className="py-2">
            ...
            <div className="d-flex justify-content-between align-items-start mb-2">
              ...
              {/* Reply Actions */}
              {(isReplyAuthor || user.role === 'admin' || user.role === 'teacher') && (
                <Dropdown>
            ...
            <div className="reply-actions d-flex align-items-center">
              <Button
                variant={isReplyLiked ? "danger" : "outline-secondary"}
                size="sm"
                onClick={() => {
                  // Handle like reply
                ...
                className="me-2"
              >
                <i className={`fas fa-heart ${isReplyLiked ? '' : 'fa-regular'}`}></i>
                {reply.likeCount > 0 && (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  alert('Reply to reply functionality can be added here');
                }}
              >
